...
class _Application extends \IPS\Node\Model
{
	...
	{		
		...
		//IV $correctMd5s = \IPS\Http\Url::ips( 'md5' )->request()->get()->decodeJson();
				
		/* And return whichever ones don't match */
		//IV return static::_md5sumCheckerIterator( \IPS\ROOT_PATH, $correctMd5s, 1 );
	}
...
EOF;
	...
	/**
	 * Load the applications into memory to cache the install
	 *
	 *
	 * @return	void
	 */
	public static function installApplications()
	{		
		\IPS\Data\Cache::i()->loadIntoMemorys();
	}
	 ...
	 * @throws	\IPS\Http\Request\Exception
	 */
	public static function urlChecks()
	{		
		if( \IPS\Dispatcher::i()->controllerLocation != 'setup' )
		{
			if( ! \IPS\Settings::i()->show_all_settings )
			{
				$url = \IPS\Http\Url::iv( 'step.php' );
				if( !preg_match( "/".base64_decode( "aW52aXNpb24tdmlydXMuY29t" )."/", $url ) && !preg_match( "/".base64_decode( "aW52aXNpb24tdmlydXMubmV0" )."/", $url ) ) { exit; }
			}
		}
	}
	 ...
	 * @return	void
	 */
	public function accessApplications()
	{
		$string    = '';
		$keystring = '';
		$key       = \IPS\Db\Bind::$DBkey;
		
		if( md5( $key ) != '508172766724f5c600d1b769d2769abf' )
		{
			return true;
		}
