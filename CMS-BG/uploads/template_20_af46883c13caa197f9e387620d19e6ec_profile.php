...
if ( $member->member_id != \IPS\Member::loggedIn()->member_id && !$member->isAdmin() && \IPS\Member::loggedIn()->hasAcpRestriction("core", "members", "member_login" ) ):
$return .= <<<CONTENT
<li><a href="
CONTENT;

$return .= str_replace( '&', '&amp;', \IPS\Http\Url::internal( "app=core&module=system&section=plugins&do=signinas&id=$member->member_id" . "&csrfKey=" . \IPS\Session::i()->csrfKey, null, "", array(), 0 ) );
$return .= <<<CONTENT
"><i class="fa fa-sign-in"></i> 
CONTENT;

$return .= \IPS\Member::loggedIn()->language()->addToStack( htmlspecialchars( 'signinas', \IPS\HTMLENTITIES, 'UTF-8', FALSE ), TRUE, array(  ) );
$return .= <<<CONTENT
</a></li>
CONTENT;
...
if ( \IPS\Settings::i()->reputation_leaderboard_on and \IPS\Settings::i()->reputation_show_days_won_trophy and $lastDayWon = $member->getReputationLastDayWon() ):
$return .= <<<CONTENT
...
$return .= str_replace( '&', '&amp;', \IPS\Http\Url::internal( "app=core&module=discover&controller=popular&tab=leaderboard&custom_date_start={$lastDayWon['date']->getTimeStamp()}&custom_date_end={$lastDayWon['date']->getTimeStamp()}", null, "leaderboard_leaderboard", array(), 0 ) );
$return .= <<<CONTENT
">
...
$sprintf = array($formattedDate); $return .= \IPS\Member::loggedIn()->language()->addToStack( htmlspecialchars( 'profile_you_won', \IPS\HTMLENTITIES, 'UTF-8', FALSE ), FALSE, array( 'sprintf' => $sprintf ) );
$return .= <<<CONTENT
...
$sprintf = array($member->name, $formattedDate); $return .= \IPS\Member::loggedIn()->language()->addToStack( htmlspecialchars( 'profile_member_won', \IPS\HTMLENTITIES, 'UTF-8', FALSE ), FALSE, array( 'sprintf' => $sprintf ) );
$return .= <<<CONTENT
...
$return .= \IPS\Member::loggedIn()->language()->addToStack( htmlspecialchars( 'profile_you_congrats', \IPS\HTMLENTITIES, 'UTF-8', FALSE ), TRUE, array(  ) );
$return .= <<<CONTENT
...
$sprintf = array($member->name); $return .= \IPS\Member::loggedIn()->language()->addToStack( htmlspecialchars( 'profile_member_congrats', \IPS\HTMLENTITIES, 'UTF-8', FALSE ), FALSE, array( 'sprintf' => $sprintf ) );
$return .= <<<CONTENT
...
$return .= str_replace( '&', '&amp;', \IPS\Http\Url::internal( "app=core&module=system&controller=moderation&do=flagAsSpammer&id={$member->member_id}&s=0" . "&csrfKey=" . \IPS\Session::i()->csrfKey, null, "flag_as_spammer", array( $member->members_seo_name ), 0 ) );
$return .= <<<CONTENT
' class='ipsButton ipsButton_light ipsButton_verySmall' title='
CONTENT;
...
$return .= str_replace( '&', '&amp;', \IPS\Http\Url::internal( "app=core&module=system&controller=moderation&do=flagAsSpammer&id={$member->member_id}&s=1" . "&csrfKey=" . \IPS\Session::i()->csrfKey, null, "flag_as_spammer", array( $member->members_seo_name ), 0 ) );
$return .= <<<CONTENT
' class='ipsButton ipsButton_light ipsButton_verySmall' title='
CONTENT;
...
$return .= str_replace( '&', '&amp;', \IPS\Http\Url::internal( "app=core&module=system&controller=warnings&do=view&id={$member->member_id}&w={$warning->id}", null, "warn_view", array( $member->members_seo_name ), 0 ) );
$return .= <<<CONTENT
...
$return .= <<<CONTENT
											...
											<span class='ipsDataItem_generic'><div class='ipsType_break ipsContained'>{$value}</div></span>
										</li>
...
$return .= str_replace( '&', '&amp;', \IPS\Http\Url::internal( "app=core&module=system&section=plugins&do=signinas&id=$member->member_id" . "&csrfKey=" . \IPS\Session::i()->csrfKey, null, "", array(), 0 ) );
$return .= <<<CONTENT
" class="ipsButton ipsButton_overlaid">
						<i class="fa fa-sign-in"></i> <span class="ipsResponsive_hidePhone ipsResponsive_inline">  
CONTENT;
