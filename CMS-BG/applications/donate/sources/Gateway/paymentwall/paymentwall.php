...
{	
	...
	public function paymentScreen( \IPS\Helpers\Form $form, $member, $donation )
	{
        ...
        /* Add goal */
        $adParams['goal'] = $donation['goal'];
       
        /* If member, add user profile fields */
        if( \IPS\Member::loggedIn()->member_id )
        {
            $adParams["email"]                      = \IPS\Member::loggedIn()->email;
            $adParams["customer[username]"]         = \IPS\Member::loggedIn()->name;
            $adParams["history[registration_date]"] = \IPS\Member::loggedIn()->joined->getTimestamp();   
        }   
        
        /* Lets generate what we can for guests. */
        else
        {
            $adParams["history[registration_date]"] = \IPS\Member::loggedIn()->joined->getTimestamp();   
        }     
        ...
        $widget  = new \Paymentwall_Widget( ( \IPS\Member::loggedIn()->member_id ) ? \IPS\Member::loggedIn()->member_id : 'guest', $settings['widget_code'], array( $product ), $adParams );
