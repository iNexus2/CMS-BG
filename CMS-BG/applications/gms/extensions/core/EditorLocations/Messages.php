...
namespace IPS\gms\extensions\core\EditorLocations;
...
class _Messages
{
	...
	{
		return \IPS\gms\Message::load( $id1 );
	}
	...
	protected function performRebuild( $offset, $max, $callback )
	{
		...
		/* Language bits */
		foreach( \IPS\Db::i()->select( '*', 'core_sys_lang_words', "word_key LIKE 'gms_message_%_message'" ) as $word )
		{
			try
			{
	...
	{
		return \IPS\Db::i()->select( 'COUNT(*)', 'core_sys_lang_words', "word_key LIKE 'gms_message_%_message'" )->first();
	}
}